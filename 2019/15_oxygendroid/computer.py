...
def runMultiple(
    programs: List[List[int]], values: List[List[int]], messages: List[str] = [""]
):
    printRocket()
    ...
    for i in range(0, len(programs)):
        ...
        while True:
            print(
                ("".join(" " * (8 - len(str(states[i][-2])) // 2)) + str(states[i][-2]))
            )
            print("")
...
def compute(
    ...
    while op != 99:
        ...
        try:
            p1 = getVal(pnt + 1, mod1)
        except:
            p1 = 0
        try:
            p2 = getVal(pnt + 2, mod2)
        except:
            p2 = 0
        try:
            p3 = getVal(pnt + 3, mod3)
        except:
            p3 = 0
        try:
            p1_a = getAdr(pnt + 1, mod1)
        except:
            pass
        try:
            p2_a = getAdr(pnt + 2, mod2)
        except:
            pass
        try:
            p3_a = getAdr(pnt + 3, mod3)
        except:
            pass
        # op-handling
        if op == 1:
